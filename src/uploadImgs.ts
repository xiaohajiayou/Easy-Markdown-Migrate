...
export async function vscUploadRelease(clip:boolean=false) {
...
export async function vscReleaseModify(clip:boolean=false) {
    ...
    await upload(clip,true);
    suspendedLogMsg();
    // analyze();

}

export async function vscOriginUpdate(clip:boolean=false) {
    if (!await upCheck()) {
        suspendedLogMsg();
        return;
    }
